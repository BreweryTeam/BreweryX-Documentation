...
:::caution[Different Server == Different Recipes]

The recipes below are the default BreweryX recipes. Many servers customize, add, or disable recipes.

If you're a player, a recipe from here may not work on the server that you're playing on.

:::

```yaml title="recipes.yml" frame="code"
